...
export default function WikiTabs() {
  ...
  return (
    <div className="flex h-full z-10 w-full">
      <Link href={pathname} className={activeClass}>
        문서
      </Link>
      ...
      <div className="flex h-full z-10 mr-4">
        <Link href={pathname} className={!isEditing ? activeClass : inactiveClass}>
          읽기
        </Link>
        <Link href={`${pathname}?mode=edit`} className={isEditing ? activeClass : inactiveClass}>
          편집
        </Link>
        <Link href={`${pathname}?mode=history`} className={isHistory ? activeClass : inactiveClass}>
          역사 보기
        </Link>
      </div>
    </div>
